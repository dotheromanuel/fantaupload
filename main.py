...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if file_size > max_file_size:
        ...
        file_upload_count = 1
    bot.editMessageText(message,'💆‍♂️Preparando Archivo🗃...')
    evidname = ''
    files = []
    if client:
    ...
    try:
        ...
        if '/uptype' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'❌Error en el comando /uptype (typo de subida (evidence,calendar,draft,blog))❌')
            return
        if '/proxy' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/start' in msgText:
            ...
            #    
                # set in debug
            #    api_id = 7289506
            #    api_hash = '5b4c06c2feefbc3d965dc7dd50077886'
            #    bot_token = '5186907110:AAEZ3oYd2tKvQo5S9APS3ez8UrV_88w_L1M'
            ...
            #    return
            bot.editMessageText(message,'🤷‍♂️No lo se Rick Dimelo Tú😵')
    except Exception as ex:
           print(str(ex))
